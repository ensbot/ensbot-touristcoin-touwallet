...
let less_srcFile = app + 'styles/mycrypto-master.less';
let less_destFolder = dist + 'css';
let less_destFolder_CX = dist_CX + 'css';
let less_destFile = 'mycrypto-master.css';
let less_destFileMin = 'mycrypto-master.min.css';
...
let js_destFile = 'mycrypto-master.js';
...
let js_destFileStatic = 'mycrypto-static.min.js';
...
gulp.task('zip', ['getVersion'], function() {
    gulp.src(dist + '**/**/*')
        ...
        .pipe(rename(function (path) {
          path.dirname = './mycrypto-' + versionNum + '/' + path.dirname;
        }))
        .pipe(zip('./mycrypto-' + versionNum + '.zip'))
...
gulp.task('travisZip', ['getVersion'], function() {
    gulp.src(dist + '**/**/*')
        .pipe(plumber({ errorHandler: onError }))
        .pipe(rename(function (path) {
          path.dirname = './touwallet-' + versionNum + '/' + path.dirname;
        }))
        .pipe(zip('./touwallet-' + versionNum + '.zip'))
