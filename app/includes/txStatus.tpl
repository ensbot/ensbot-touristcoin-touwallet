<main class="tab-pane block--container active" ng-if="globalService.currentTab==globalService.tabs.txStatus.id" ng-controller='txStatusCtrl' role="main" ng-cloak>
    ...
    <article class="row">
      ...
      <section class="col-xs-12 col-sm-6 col-sm-offset-3 text-center">
          ...
          <button tabindex="0"
                  ...
                  translate="NAV_CheckTxStatus"> TX Status </button>
      </section>
    </article>
  </section>
  <!-- / Section 1 -->
  ...
  <section class="block txstatus__2" ng-show="txInfo.status==txStatus.mined || txInfo.status==txStatus.notFound || txInfo.status==txStatus.found">
    ...
    <div class="cont-md" ng-show="txInfo.status == txStatus.mined">
      ...
      <h5> <a href="https://explorer.touristcoin.net/tx/{{ tx.hash }}" target="_blank" rel="noopener noreferrer"> {{ tx.hash }} </a> </h5>
      ...
      <table class="table table-striped txstatus__2 cont-md"> <!-- ng-show="tx.status=='foundOnChain' || foundInPending'"-->
        <tbody>
          <tr>
            ...
            <td>
              <a href="https://explorer.touristcoin.net/tx/{{ txInfo.hash }}" target="_blank" rel="noopener noreferrer">
                {{ txInfo.hash }}
              </a>
            </td>
          </tr>
          <tr>
            ...
            <td>
              <a href="https://explorer.touristcoin.net/addr/{{ txInfo.from }}" target="_blank" rel="noopener noreferrer">
                {{ txInfo.from }}
              </a>
            </td>
          </tr>
          <tr>
            ...
            <td>
              <a href="https://explorer.touristcoin.net/addr/{{ txInfo.to }}" target="_blank" rel="noopener noreferrer">
                {{ txInfo.to }}
              </a>
            </td>
          </tr>
