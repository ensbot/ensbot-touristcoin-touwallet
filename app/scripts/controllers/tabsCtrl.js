...
var tabsCtrl = function($scope, globalService, $translate, $sce) {
    ...
    $scope.gasChanged = function() {
        ...
        $scope.gasPriceMsg = ethFuncs.gasAdjustment < 10 ? true : false
    }
    ...
    $scope.changeNode = function(key) {
        ...
        ajaxReq.getCurrentBlock(function(data) {
            if (data.error) {
                ...
                $scope.nodeIsConnected = true;
                $scope.notifier.info( globalFuncs.successMsgs[5] + '<br /> Network: <strong>' + $scope.nodeType + ' </strong> provided by <strong>' + $scope.nodeService + '</strong>', 10000)
            }
        });
